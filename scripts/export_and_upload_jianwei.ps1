param(
    ...
    [string]$RemoteHost = $env:JIANWEI_REMOTE_HOST,
    [string]$RemoteArtifactsRoot = $(if ($env:JIANWEI_REMOTE_ARTIFACTS_ROOT) { $env:JIANWEI_REMOTE_ARTIFACTS_ROOT } else { "/root/workspace/vertical_ai_news/artifacts" }),
    [string]$RemoteWebDir = $(if ($env:JIANWEI_REMOTE_WEB_DIR) { $env:JIANWEI_REMOTE_WEB_DIR } else { "/root/workspace/vertical_ai_news/jianwei_web" }),
...
if (-not $RemoteHost) {
    throw "请通过 -RemoteHost 或环境变量 JIANWEI_REMOTE_HOST 指定服务器，例如：root@129.204.144.110"
}
